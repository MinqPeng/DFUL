'''
cost.py: contains all cost functions (and related helper functions) for siamese network and unsupervised learning
'''
import numpy as np
...
def squared_distance(X, Y=None, W=None):
    ...
    Xval = K.variable(X)
    sum_dimensions = list(range(2, K.ndim(Xval) + 1))
...
def knn_affinity(X, n_nbrs, scale=None, scale_nbr=None, local_scale=None, verbose=False):
    ...
    if scale is None:
        ...
        if scale_nbr is None:
            ...
            print("get Affinity scale_nbr, n_nbrs:", scale_nbr, n_nbrs)
...
def full_affinity(X, scale):
    ...
    sigma_squared = tf.cast(sigma_squared, dtype=tf.float32)
...
def get_vae_loss(z_mean, z_log_var, image_size):
