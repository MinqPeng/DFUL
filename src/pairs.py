...
def create_pairs_from_embed_data(x1, y, x2, p=None, k=5, tot_pairs=None, precomputed_knn_path='', use_approx=False, pre_shuffled=False, verbose=None):
    '''
    ...
    x2: parallel data array (pairs will exactly shadow the indices of x2,
        but be drawn from x1)
    ...
    returns:    pairs for x2, labels(inferred by knn), (labels_true, the absolute truth, if y
                is provided
    '''
    print('[build pairwise dataset]')
    ...
    if verbose:
        ...
        print("$ ks:", n, k_max, k, pairs_per_pt) #-1,2,2,2
    ...
    for i in range(n):
        ...
        # form the pairs
        new_pos = [[x2[i], x2[c]] for c in choices]
        if y is not None:
        ...
        try:
            ...
            continue
        # form negative pairs
        new_neg = [[x2[i], x2[c]] for c in choices]
        if y is not None:
    ...
    ret = [np.array(pairs).reshape((len(pairs), 2) + x2.shape[1:])]
    ...
    if y is not None:
        ...
        if verbose:
            ...
            print("$ confusion matrix in coding space for pairs and approximated labels:")
            cm = metrics.confusion_matrix(true, labels)
            print(cm)
            print(cm/true.shape[0])
            print("$ pairs number is %d" %true.shape[0])
            
        ret.append(true.reshape(-1))

    return ret

def create_pairs_from_labeled_data(x, label, y, use_classes=None):
    '''
    ...
    clu =  multiclass.unique_labels(label)
    print(clu)
    indice =[[] for i in range(len(clu))]
    for i in range(len(label)):
        indice[label[i].astype(int)].append(i)
    n_clusters = len(indice)
    if use_classes == None:
    ...
    prelabel = []
    true = []
    n = min([len(indice[d]) for d in range(n_clusters)]) - 1
    for d in use_classes:
        for i in range(n):
            z1, z2 = indice[d][i], indice[d][i + 1]
            pairs += [[x[z1], x[z2]]]
            prelabel += [1]
            if y is not None:
                plabel = [y[z1] == y[z2]]
    ...
    if y is not None:
        true = np.array(true).astype(np.int).reshape(-1)
        print("# quality report")
        print("$ confusion matrix with clustering result for pairs and approximated labels:")
        cm = metrics.confusion_matrix(true, prelabel)
        print(cm)
        print(cm/true.shape[0])
        print("$ pairs number is %d" %true.shape[0])
    
    return pairs, prelabel, true