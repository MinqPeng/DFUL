...
def main():
    ...
    # get the dataset
    dataset = get_dataset(args.dataset) #C10 = 1

    train_loader, test_loader, _ = build_dataset(dataset=dataset,
    ...
    test_stats1 = AverageMeterSet()
    test(model, train_loader, torch_device, test_stats1)
    stat_str1 = test_stats1.pretty_string(ignore=model.tasks)
    print(stat_str1)
    test_stats2 = AverageMeterSet()
    test(model, test_loader, torch_device, test_stats2)
    stat_str2 = test_stats2.pretty_string(ignore=model.tasks)
    print(stat_str2)
