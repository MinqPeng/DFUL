...
def get_scale(x, scale_nbrs):
    ...
    scale_nbrs:     k in the aforementeiond calculation.
    ...
    *note:      the scale is the variance term of the gaussian
    ...
    nbrs = NearestNeighbors(n_neighbors=scale_nbrs).fit(sample)
    distances, _ = nbrs.kneighbors(sample)
    # return the median distance
    return np.median(distances[:, scale_nbrs - 1])
